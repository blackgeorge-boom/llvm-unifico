...
///   {8-6}  = shifter
///   {5-0}  = imm
static inline unsigned getShifterImm(ShiftExtendType ST,
                                     unsigned Imm) {
  assert((Imm & 0x3f) == Imm && "Illegal shifted immedate value!");
  unsigned STEnc = 0;
  switch (ST) {
  default:  llvm_unreachable("Invalid shift requested");
  case LSL: STEnc = 0; break;
  case LSR: STEnc = 1; break;
  case ASR: STEnc = 2; break;
  case ROR: STEnc = 3; break;
  case MSL: STEnc = 4; break;
  }
...
static bool canUseOrr(uint64_t Chunk, uint64_t &Encoding) {
  ...
  return processLogicalImmediate(Chunk, 64, Encoding);
}
...
static inline void expandMOVImmSimple(uint64_t Imm, unsigned BitSize,
  ...
  if (BitSize == 32) {
    Imm &= (1LL << 32) - 1;
    FirstOpc = (isNeg ? X86::MOV64rm : X86::MOV64rm);
  } else {
    FirstOpc = (isNeg ? X86::MOV64rm : X86::MOV64rm);
  }
  ...
  unsigned Opc = (BitSize == 32 ? X86::MOV64rm : X86::MOV64rm);
...
static bool tryToreplicateChunks(uint64_t UImm,
  ...
  for (CountMap::const_iterator Chunk = Counts.begin(), End = Counts.end();
    ...
    Insn.push_back({ X86::OR64rm, 0, Encoding });
    ...
    Insn.push_back({ X86::MOV64rm, Imm16,
                    getShifterImm(LSL, ShiftAmt) });
...
static bool trySequenceOfOnes(uint64_t UImm,
  ...
  uint64_t Encoding = 0;
  processLogicalImmediate(OrrImm, 64, Encoding);
  Insn.push_back({ X86::OR64rm, 0, Encoding });

  const bool SingleMovk = SecondMovkIdx == NotSet;
  Insn.push_back({ X86::MOV64rm, getChunk(UImm, FirstMovkIdx),
                  getShifterImm(LSL,
                                            FirstMovkIdx * 16) });
  ...
  Insn.push_back({ X86::MOV64rm, getChunk(UImm, SecondMovkIdx),
                  getShifterImm(LSL,
                                            SecondMovkIdx * 16) });
...
void expandMOVImm(uint64_t Imm, unsigned BitSize,
  ...
  if (processLogicalImmediate(UImm, BitSize, Encoding)) {
    unsigned Opc = (BitSize == 32 ? X86::OR64rm : X86::OR64rm);
  ...
  for (unsigned Shift = 0; Shift < BitSize; Shift += 16) {
    ...
    if (processLogicalImmediate(ZeroChunk, BitSize, Encoding) ||
        ...
        processLogicalImmediate(ReplicateChunk, BitSize,
                                            Encoding)) {
      // Create the ORR-immediate instruction.
      Insn.push_back({ X86::OR64rm, 0, Encoding });
...
int X86TTIImpl::getIntImmCost(int64_t Val) {
  ...
  if (ST->hasAArch64ConstantCostModel()) {
    ...
    if (Val == 0 || isLogicalImmediate(Val, 64))
      return 0;
